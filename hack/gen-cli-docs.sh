# Generate the Jackal CLI docs
printf "Generating CLI docs\n"
JACKAL_CONFIG=hack/empty-config.toml go run main.go internal gen-cli-docs

# Create the top menu label for the sidebar menu
printf "Generating sidebar menu label\n"
MENU_LABEL='{"label": "CLI Commands"}\n'
printf "${MENU_LABEL}" > docs/2-the-jackal-cli/100-cli-commands/_category_.json
...
for FILE in $(find docs/2-the-jackal-cli/100-cli-commands -name "*.md")
do
  sed -i.bak 's/^##/#/g' ${FILE}
