...
func TestCreateSBOM(t *testing.T) {
	...
	require.FileExists(t, filepath.Join(sbomPath, "dos-games", "sbom-viewer-docker.io_defenseunicorns_jackal-game_multi-tile-dark.html"))
	require.FileExists(t, filepath.Join(sbomPath, "dos-games", "compare.html"))
	require.FileExists(t, filepath.Join(sbomPath, "dos-games", "docker.io_defenseunicorns_jackal-game_multi-tile-dark.json"))
	...
	require.NoError(t, err, stdOut, stdErr)
	// Test that the game package generates the SBOMs we expect (images only)
	_, err = os.ReadFile(filepath.Join(sbomPath, "dos-games", "sbom-viewer-docker.io_defenseunicorns_jackal-game_multi-tile-dark.html"))
	require.NoError(t, err)
	_, err = os.ReadFile(filepath.Join(sbomPath, "dos-games", "compare.html"))
	require.NoError(t, err)
	_, err = os.ReadFile(filepath.Join(sbomPath, "dos-games", "docker.io_defenseunicorns_jackal-game_multi-tile-dark.json"))
	require.NoError(t, err)
