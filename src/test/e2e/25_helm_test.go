...
func TestHelm(t *testing.T) {
	...
	helmChartsPkg = filepath.Join("build", fmt.Sprintf("jackal-package-helm-charts-%s-0.0.1.tar.zst", e2e.Arch))
...
func testHelmChartsExample(t *testing.T) {
	...
	stdOut, stdErr, err := e2e.Jackal("package", "create", localTgzChartPath, "--tmpdir", tmpdir, "--confirm")
	require.NoError(t, err, stdOut, stdErr)
	defer e2e.CleanFiles(fmt.Sprintf("jackal-package-helm-charts-local-tgz-%s-0.0.1.tar.zst", e2e.Arch))

	// Create a package that needs dependencies
	evilChartDepsPath := filepath.Join("src", "test", "packages", "25-evil-chart-deps")
	stdOut, stdErr, err = e2e.Jackal("package", "create", evilChartDepsPath, "--tmpdir", tmpdir, "--confirm")
	...
	stdOut, stdErr, err = e2e.Jackal("package", "create", evilChartLookupPath, "--tmpdir", tmpdir, "--confirm")
	require.Error(t, err, stdOut, stdErr)
	...
	stdOut, stdErr, err = e2e.Jackal("package", "create", "examples/helm-charts", "-o", "build", "--registry-override", "ghcr.io/stefanprodan=docker.io/stefanprodan", "--tmpdir", tmpdir, "--confirm")
	require.NoError(t, err, stdOut, stdErr)
	...
	stdOut, stdErr, err = e2e.Jackal("package", "create", "examples/helm-charts", "-o", "build", "--registry-override", "ghcr.io=localhost:555/noway", "--tmpdir", tmpdir, "--confirm")
	require.Error(t, err, stdOut, stdErr)
	...
	stdOut, stdErr, err = e2e.Jackal("package", "create", "examples/helm-charts", "-o", "build", "--registry-override", "ghcr.io/stefanprodan=localhost:555", "--tmpdir", tmpdir, "--confirm")
	require.Error(t, err, stdOut, stdErr)
	...
	stdOut, stdErr, err = e2e.Jackal("package", "create", "examples/helm-charts", "-o", "build", "--registry-override", "ghcr.io=docker.io", "--tmpdir", tmpdir, "--confirm")
	require.NoError(t, err, stdOut, stdErr)

	// Deploy the example package.
	stdOut, stdErr, err = e2e.Jackal("package", "deploy", helmChartsPkg, "--confirm")
	require.NoError(t, err, stdOut, stdErr)
	...
	stdOut, stdErr, err = e2e.Jackal("package", "remove", "helm-charts", "--confirm")
	require.NoError(t, err, stdOut, stdErr)
}
...
func testHelmEscaping(t *testing.T) {
	...
	// Create the package.
	stdOut, stdErr, err := e2e.Jackal("package", "create", "src/test/packages/25-evil-templates/", "--confirm")
	require.NoError(t, err, stdOut, stdErr)

	path := fmt.Sprintf("jackal-package-evil-templates-%s.tar.zst", e2e.Arch)
	...
	// Remove the package.
	stdOut, stdErr, err = e2e.Jackal("package", "remove", "evil-templates", "--confirm")
	require.NoError(t, err, stdOut, stdErr)
}
...
func testHelmUninstallRollback(t *testing.T) {
	...
	goodPath := fmt.Sprintf("build/jackal-package-dos-games-%s-1.0.0.tar.zst", e2e.Arch)
	evilPath := fmt.Sprintf("jackal-package-dos-games-%s.tar.zst", e2e.Arch)

	// Create the evil package (with the bad service).
	stdOut, stdErr, err := e2e.Jackal("package", "create", "src/test/packages/25-evil-dos-games/", "--skip-sbom", "--confirm")
	require.NoError(t, err, stdOut, stdErr)

	// Deploy the evil package.
	stdOut, stdErr, err = e2e.Jackal("package", "deploy", evilPath, "--timeout", "10s", "--confirm")
	require.Error(t, err, stdOut, stdErr)
	...
	require.NotContains(t, string(helmOut), "jackal-f53a99d4a4dd9a3575bedf59cd42d48d751ae866")

	// Deploy the good package.
	stdOut, stdErr, err = e2e.Jackal("package", "deploy", goodPath, "--confirm")
	require.NoError(t, err, stdOut, stdErr)

	// Ensure that this does create a dos-games chart
	helmOut, err = exec.Command("helm", "list", "-n", "dos-games").Output()
	require.NoError(t, err)
	require.Contains(t, string(helmOut), "jackal-f53a99d4a4dd9a3575bedf59cd42d48d751ae866")
	...
	helmOut, err = exec.Command("helm", "history", "-n", "dos-games", "jackal-f53a99d4a4dd9a3575bedf59cd42d48d751ae866", "--max", "1").Output()
	require.NoError(t, err)
...
func testHelmAdoption(t *testing.T) {
	...
	packagePath := fmt.Sprintf("build/jackal-package-dos-games-%s-1.0.0.tar.zst", e2e.Arch)
	deploymentManifest := "src/test/packages/25-manifest-adoption/deployment.yaml"

	// Deploy dos-games manually into the cluster without Jackal
	kubectlOut, _, _ := e2e.Kubectl("apply", "-f", deploymentManifest)
	require.Contains(t, string(kubectlOut), "deployment.apps/game created")

	// Deploy dos-games into the cluster with Jackal
	stdOut, stdErr, err := e2e.Jackal("package", "deploy", packagePath, "--confirm", "--adopt-existing-resources")
	require.NoError(t, err, stdOut, stdErr)
